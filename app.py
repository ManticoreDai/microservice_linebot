...
def movies(event):
    ...
    if result_rank:
        ...
        line_bot_api.reply_message(event.reply_token, FlexSendMessage('Movie Introduction',FlexMessage))

    #value_theater = re.compile(r'^排行榜 *(2[0]|1[0-9]|[1-9])$')
    if event.message.text == "電影院屏東":
        
        FlexMessage = json.load(open('theater_Pingtung.json','r',encoding='utf-8'))
        line_bot_api.reply_message(event.reply_token, FlexSendMessage('Theater Location',FlexMessage))
        
    elif event.message.text == "電影院高雄":
        FlexMessage = json.load(open('theater_Kaohsiung.json','r',encoding='utf-8'))
        line_bot_api.reply_message(event.reply_token, FlexSendMessage('Theater Location',FlexMessage))

@handler.add(PostbackEvent)
def handle_postback(event):
    id = int(event.postback.data)
    if id > len(movies_df):
        id = len(movies_df)
